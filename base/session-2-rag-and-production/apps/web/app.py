...
st.title("🥗 FridgeAI")
st.caption(
    "Discover recipes by meaning and get recommendations "
    "grounded in your recipe collection."
)
...
def display_request_error(
    ...
    if error.response is not None:
        reference = error.response.headers.get("X-Request-ID")
        try:
            detail = error.response.json().get("detail")
        except (requests.exceptions.JSONDecodeError, AttributeError):
            pass

    message = detail or f"Could not {action}. Please try again."
    if reference:
...
with st.sidebar:
    ...
    try:
        ...
        if readiness_response.ok:
            ...
            st.warning("Some services are unavailable")

        for service, service_status in readiness.get("checks", {}).items():
            icon = "✅" if service_status == "healthy" else "❌"
            st.caption(f"{icon} {service.title()}")
    except (requests.RequestException, ValueError):
        st.error("API unavailable. Check Docker Compose and try again.")
...
with browse_tab:
    ...
    if st.button("Load recipes", type="primary"):
        try:
            ...
            display_request_error("load recipes", error)
...
with search_tab:
    ...
    if search_submitted:
        if not search_query.strip():
            ...
            try:
                with st.spinner("Finding recipes by meaning..."):
                    response = requests.get(
                ...
                display_request_error("search recipes", error)
    ...
    if recommendation_submitted:
        if not recommendation_query.strip():
            ...
            try:
                ...
                display_request_error("generate a recommendation", error)
...
with add_tab:
    ...
    if submitted:
        ...
        try:
            ...
            display_request_error("add the recipe", error)
