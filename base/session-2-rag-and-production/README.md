...
Session 2 adds grounded RAG and asynchronous indexing to the application, then makes it a resilient, observable, continuously verified portfolio project.
...
- Build grounded recommendations from retrieved recipe context and return sources.
- Move embedding and indexing work into retryable Celery tasks using Redis.
...
- Root-level GitHub Actions CI runs Ruff and pytest against PostgreSQL for Session 2 changes.
...
The CI workflow at `../../.github/workflows/session-2-ci.yml` runs Ruff and the PostgreSQL-backed test suite for relevant pushes and pull requests.
