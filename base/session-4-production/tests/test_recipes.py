...
def test_recommendation_returns_404_without_sources(
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
    ...
    response = client.post(
        "/api/v1/recommendations",
        json={"query": "a meal that does not exist", "limit": 3},
    )

    assert response.status_code == 404
    assert response.json() == {"detail": "No matching recipes found"}


def test_recommendation_returns_503_when_model_is_unavailable(
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    create_response = client.post(
    ...
    monkeypatch.setattr(
        "apps.api.recommendations.search_recipes",
        lambda query, limit: [SimpleNamespace(id=recipe_id, score=0.9)],
    )
    monkeypatch.setattr(
        "apps.api.recommendations.generate_recommendation",
        lambda query, recipes: (_ for _ in ()).throw(
    ...
    response = client.post(
        "/api/v1/recommendations",
        json={"query": "a warming vegan dinner", "limit": 3},
        headers={"X-Request-ID": "model-outage-test"},
    )
    ...
    assert response.headers["X-Request-ID"] == "model-outage-test"
